...
def delete_commentaire(commentaire_id: int, conn: sqlite3.Connection = Depends(get_db)):
    ...
    cursor.execute("DELETE FROM sqlite_sequence WHERE name='commentaires'")
    conn.commit()
    if cursor.rowcount == 0:
        raise HTTPException(status_code=404, detail="Commentaire non trouvé")
...
def clear_all(conn: sqlite3.Connection = Depends(get_db)):
    cursor = conn.cursor()
    cursor.execute("PRAGMA foreign_keys = OFF;")
    tables = ['posts', 'categories', 'commentaires', 'utilisateurs']
    for table in tables:
    ...
    cursor.execute("DELETE FROM sqlite_sequence WHERE name='categories'")
    conn.commit()
    return {"message": "Toutes les données ont été effacées avec succès"}
